...
export async function handleSaveDebrief(req: Request, env: Env): Promise<Response> {
  try {
    ...
    const debrief: QuinnDebrief = {
      ...
      parts_used: body.parts_used ? JSON.stringify(body.parts_used) : null,
      restock_needed: body.restock_needed ? JSON.stringify(body.restock_needed) : null,
      returns_needed: body.returns_needed ? JSON.stringify(body.returns_needed) : null,
      ...
      equipment_missed: body.equipment_missed ? JSON.stringify(body.equipment_missed) : null,
      property_notes: body.property_notes,
      recommendations_observed: body.recommendations_observed ? JSON.stringify(body.recommendations_observed) : null,
